...
<div class="col-md-6"><br>
    ...
    <form class="login-form" action="{{route($route ?? 'coodinator.student.biodata.update',[$student->admission->id])}}" method="post" enctype="multipart/form-data">
        @csrf
        ...
        <div class="form-group">
            ...
            <select name="schedule" class="form-control">
                <option value="{{$student->admission->schedule->id}}">{{$student->admission->schedule->name}}</option>
                @foreach(administrator()->schedules() as $schedule)
                    @if($student->admission->schedule->id != $schedule->id)
